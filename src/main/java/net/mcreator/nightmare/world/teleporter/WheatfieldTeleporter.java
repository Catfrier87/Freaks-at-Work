...
@EventBusSubscriber
public class WheatfieldTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(NightmareModBlocks.THE_WHEATFIELD_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("nightmare:the_wheatfield_portal"), poiType);
	...
	public WheatfieldTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = NightmareModBlocks.THE_WHEATFIELD_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
