...
public class TaserItem extends Item {
	public TaserItem(Item.Properties properties) {
		super(properties);
	}
	...
	private ItemStack findAmmo(Player player) {
		ItemStack stack = ProjectileWeaponItem.getHeldProjectile(player, e -> e.getItem() == TaserShotEntity.PROJECTILE_ITEM.getItem());
		if (stack == ItemStack.EMPTY) {
			for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
				ItemStack teststack = player.getInventory().getItem(i);
				if (teststack != null && teststack.getItem() == TaserShotEntity.PROJECTILE_ITEM.getItem()) {
					stack = teststack;
					break;
				}
			}
